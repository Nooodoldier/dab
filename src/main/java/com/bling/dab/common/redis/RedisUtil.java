...
public class RedisUtil{
    ...
    public static int DEFAULT_EXPIRED_IN_SECONDS = 60 * 60 * 24 * 30;
    /**
     * redis对象实例
     */
    ...
    private void init(RedisConfig configRead) {
        ...
        try {
            ...
            setJedisPool(new JedisPool(config, host, Integer.parseInt(port), 30000, pwd));
    ...
    private RedisUtil(RedisConfig configRead) {
        init(configRead);
    }
     ...
     * @return
     */
    public static RedisUtil getInstance(RedisConfig configRead) {

        if (redisUtil == null) {
            synchronized (RedisUtil.class) {
                if (redisUtil == null){
                    redisUtil = new RedisUtil(configRead);
                }
            }
        }
        return redisUtil;
